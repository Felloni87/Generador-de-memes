...
// INICIALIZACIONES
  const inicializarTemas = () => {
    $('dark-button').addEventListener('click', cambiarModoClaro)
    $('light-button').addEventListener('click', cambiarModoOscuro)
  }

  const inicializarPaneles = () => {
    $('aside-image').addEventListener('click', () => {
      mostrarPanelImagen()
      mostrarPanel()
    })
    ...
    $('aside-close-button').addEventListener('click', ocultarPanel)
  }
